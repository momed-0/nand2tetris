...
/*Constructor defining the predefined symbols for Comp,Dest,Jump*/
Parser::Parser() {
    compDict.clear();
    destDict.clear();
    jumpDict.clear();
...
/*Parse instruction one by one and check if its A or C*/
...
string Parser::parseAInst(string inst) {
    ...
    if (inst.find_first_not_of("0123456789") == string::npos) {
        ...
        //else check the address in  the symbol table
...
string Parser::parseCInst(string inst) {
    ...
    string instDest,instComp,instJump;
    if(idxsc == string::npos) {
        //if ; not found then no JUMP;
        instJump = "null";
        instDest = (idxe == string::npos) ? "null" : inst.substr(0,idxe);
        instComp = (idxe == string::npos) ? "0" : inst.substr(idxe + 1,inst.size() - idxe -1);
    }
    else if(idxe == string::npos) {
        //if = not found
        instDest = "null";
        instComp = inst.substr(0,idxsc);
        instJump = inst.substr(idxsc+1,inst.size() - idxsc);
    }
    else {
        instDest = inst.substr(0, idxe);
        instComp = inst.substr(idxe + 1, idxsc - idxe - 1);
        instJump = inst.substr(idxsc + 1, inst.size() - 1);
    }
    // query lookup table
